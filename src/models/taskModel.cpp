...
Task TaskModel::getRecord(int row)
{
    ...
    task.setTitle(this->data(this->index(row, 2), rTitle).toString());
    task.setDescription(this->data(this->index(row, 3), rTitle).toString());
    task.setItemId(this->data(this->index(row, 4), rItemID).toInt());
...
bool TaskModel::setRecord(int taskIndex, QString dueDate, QString title, QString description, int itemId)
{
    Database db;
